...
public class MongoDB {
    ...
    public static boolean userExists(String userId) {
        MongoDatabase mongoDatabase = mongoClient.getDatabase(TELEGRAM_DATABASE);
        MongoCollection<Document> customersCollections = mongoDatabase.getCollection(CUSTOMERS_COLLECTION);

        Document userDoc = customersCollections.find(eq("_id", userId)).first();
